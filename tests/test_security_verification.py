...
class TestCredentialSecurity:
    ...
    def test_password_never_in_login_response(self, client, broker):
        ...
        response = client.post(
            "/api/broker/login", json={"username": "test_user", "password": password}
        )
    ...
    def test_totp_code_never_in_verify_response(self, client, broker):
        ...
        totp_code = "987654"
        response = client.post("/api/broker/verify-totp", json={"code": totp_code})
...
class TestSessionTokenSecurity:
    ...
    def test_session_token_never_in_totp_response(self, client, broker):
        ...
        response = client.post("/api/broker/verify-totp", json={"code": "123456"})
...
class TestInputValidation:
    ...
    def test_login_validates_username_required(self, client):
        ...
        response = client.post("/api/broker/login", json={"password": "test_password"})
        assert response.status_code == 400

    def test_login_validates_password_required(self, client):
        """Verify login validates password is required."""
        response = client.post("/api/broker/login", json={"username": "test_user"})
        assert response.status_code == 400
    ...
    def test_login_validates_json_structure(self, client):
        ...
        response = client.post(
            "/api/broker/login", data="invalid json", content_type="application/json"
        )
        assert response.status_code == 400
...
class TestErrorHandlingSecurity:
    ...
    def test_login_error_no_stack_trace(self, client):
        ...
        response = client.post("/api/broker/login", json={"username": "test", "password": "wrong"})
    ...
    def test_totp_error_no_stack_trace(self, client, broker):
        ...
        response = client.post("/api/broker/verify-totp", json={"code": "000000"})
    ...
    def test_generic_error_messages(self, client):
        ...
        response = client.post(
            "/api/broker/login", json={"username": "nonexistent_user", "password": "wrong_password"}
        )
    ...
    def test_no_sensitive_data_in_any_response(self, client, broker):
        ...
        endpoints = [
            (
                "POST",
                "/api/broker/login",
                {"username": "test", "password": "super_secret_password"},
            ),
...
class TestLoggingSecurity:
    ...
    def test_login_does_not_log_password(self, client, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            client.post(
                "/api/broker/login",
                json={"username": "test_user", "password": "super_secret_password"},
            )
    ...
    def test_totp_does_not_log_code(self, client, broker, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            client.post("/api/broker/verify-totp", json={"code": "987654"})
...
class TestSQLInjectionSecurity:
    ...
    def test_login_prevents_sql_injection(self, client, broker):
        ...
        for attempt in injection_attempts:
            response = client.post(
                "/api/broker/login", json={"username": attempt, "password": "test"}
            )
