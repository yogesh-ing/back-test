...
def test_zero_pnl_trade_is_flat_not_a_win():
    stats = trade_stats(walk_trades(_series([1000, 1000, 1000, 1000]),
                                     _series([0, 1, 1, 0]), _series([100, 100, 100, 100])))
...
def test_trade_ids_are_sequential_and_rows_carry_dates():
    ...
    for t in trades:
        ...
        assert {"id", "entry_date", "exit_date", "side", "entry", "exit", "pnl",
                "result", "is_open"} == set(t.to_dict())
...
def test_metrics_win_rate_tracks_the_winning_trades():
    ...
    """
    result = _run([1, 0, 1, 0, 1, 0], [100, 110, 110, 100, 100, 130],
...
@pytest.mark.parametrize(
    "strategy,params",
    [("sma_crossover", {"fast": 5, "slow": 15}), ("rsi_reversion", {}),
     ("buy_and_hold", {}), ("donchian_breakout", {})],
)
def test_adapter_card_and_table_agree_on_every_strategy(strategy, params):
    ...
    result = run_on_candles(candles, strategy, params, "DEMO",
...
def test_result_dict_keys_are_backwards_compatible():
    ...
    for key in ("total_return", "cagr", "volatility", "sharpe", "max_drawdown",
                "calmar", "num_trades", "win_rate", "exposure", "final_equity", "bars"):
        assert key in m
    for key in ("closed_trades", "open_trades", "winning_trades", "losing_trades",
                "realised_pnl", "avg_trade_pnl", "best_trade_pnl", "worst_trade_pnl"):
