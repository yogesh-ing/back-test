...
def test_validate_bar_valid():
    ...
    result = validator.validate_bar({"symbol": "INFY", "open": 100, "high": 100, "low": 100, "close": 100, "volume": 0})
    assert result.valid is True
...
def test_validate_bar_invalid_missing_field():
    ...
    result = validator.validate_bar({"symbol": "INFY", "open": 100, "high": 101, "low": 99})  # missing close
...
def test_validate_bar_invalid_price():
    ...
    result = validator.validate_bar({"symbol": "INFY", "open": 0, "high": 101, "low": 99, "close": 100, "volume": 1000})
    assert result.valid is False
    assert "price" in result.code

    result = validator.validate_bar({"symbol": "INFY", "open": -10, "high": 101, "low": 99, "close": 100, "volume": 1000})
    assert result.valid is False
...
def test_validate_tick_valid():
    ...
    result = validator.validate_tick({"symbol": "INFY", "bid": 99, "ask": 101, "last": 100, "volume": 100})
    assert result.valid is True
...
def test_validate_tick_negative_volume():
    ...
    result = validator.validate_tick({"symbol": "INFY", "bid": 99, "ask": 101, "last": 100, "volume": -10})
    assert result.valid is False
...
def test_future_timestamp():
    validator = DataValidator(config={"allow_future_timestamp": False, "future_tolerance_seconds": 60})
...
def test_gap_detection_in_bar():
    ...
    result = validator.validate_bar({"symbol": "INFY", "timestamp": ts1, "open": 100, "high": 101, "low": 99, "close": 100, "volume": 1000})
    assert result.valid is True

    result = validator.validate_bar({"symbol": "INFY", "timestamp": ts2, "open": 100, "high": 101, "low": 99, "close": 100, "volume": 1000})
    assert result.valid is False
    assert result.code == "time_gap"
...
def test_consecutive_failures_alert():
    ...
    for _ in range(3):
        validator.validate_bar({"symbol": "INFY", "open": 0, "high": 101, "low": 99, "close": 100, "volume": 1000})
...
def test_generic_validate():
    ...
    # Single bar
    assert validator.validate({"symbol": "INFY", "open": 100, "high": 101, "low": 99, "close": 100, "volume": 1000}) is True

    # Single tick
    assert validator.validate({"symbol": "INFY", "bid": 99, "ask": 101, "last": 100}) is True

    # Mapping
    assert validator.validate({"INFY": {"symbol": "INFY", "open": 100, "high": 101, "low": 99, "close": 100, "volume": 1000}}) is True

    # Invalid mapping
    assert validator.validate({"INFY": {"symbol": "INFY", "open": 0, "high": 101, "low": 99, "close": 100, "volume": 1000}}) is False
...
def test_stats_and_reset():
    ...
    validator.validate_bar({"symbol": "INFY", "open": 100, "high": 101, "low": 99, "close": 100, "volume": 1000})
    validator.validate_bar({"symbol": "INFY", "open": 0, "high": 101, "low": 99, "close": 100, "volume": 1000})
