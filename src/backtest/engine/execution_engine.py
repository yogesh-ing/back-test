...
class ExecutionEngine:
    ...
    def _resolve_lot_size(self, underlying: str) -> int:
        ...
        Architecture §2: lot_size is resolved from instrument master at call time —
        never stored in playbook.
        """
    ...
    def _check_risk(
        ...
        if estimated_loss > max_loss:
            return False, (
                f"estimated max loss ₹{estimated_loss:,.0f} > limit ₹{max_loss:,.0f} (risk cap)"
            )

        # Position cap
        if (
            envelope
            and envelope.max_positions is not None
            and current_positions >= envelope.max_positions
        ):
    ...
    def _check_live_margin(
        ...
        if broker is None:
            logger.debug(
                "Live mode — no live_broker configured, skipping margin check for %s",
                signal.underlying,
            )
            return None
    ...
    def execute(
        ...
        if not signal.underlying:
            return OrderRejected(
                ...
                data_source=source,
                detail="underlying missing — C2 violation",
            )
        ...
        allowed, reason = self._check_risk(
            playbook, signal, spot_price, lot_size, current_positions
        )
        if not allowed:
        ...
        if mode == "live":
            ...
            if data_source_label == "synthetic-fallback":
                ...
                return OrderRejected(
                    ...
                    data_source=data_source_label,
                    detail=(
                        "live mode requires authenticated broker session, " "got synthetic-fallback"
                    ),
                )

            # U2.3: broker margin query before order; margin failure → OrderRejected("margin")
            margin_check = self._check_live_margin(
                signal, playbook, spot_price, lot_size, data_source_label
            )
        ...
        try:
            if signal.is_option():
                ...
                try:
                    ...
                    logger.debug(
                        "A3 seam build_intent_from_view failed, using placeholder: %s", exc
                    )
                    intent = {
                        "underlying": signal.underlying,
                        "spot": spot_price,
        ...
        try:
            if mode == "paper":
                ...
                # For U2.1, we return Fill with live label if session valid,
                # else OrderRejected handled above
                self.executed_count += 1
                return Fill(
                    intent=intent,
                    data_source=data_source_label,
