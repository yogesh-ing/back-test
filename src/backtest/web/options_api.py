...
def reset_option_state() -> None:
    """Reset the singleton broker/manager/quotes (used by tests)."""
    global _broker, _expiry_manager, _quote_provider
    _broker = None
...
def _open_trade_payload() -> tuple[dict[str, Any] | None, str | None]:
    ...
    return {"underlying": underlying, "structure_type": structure_type, "quantity": quantity, "config": config}, None
...
def _execute_trade(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    strikes = _resolve_strikes(payload["structure_type"], underlying, chain, spot, payload["config"])
    if not strikes:
    ...
    risk = PreTradeRiskCheck(capital=float(broker.capital)).check(
        ...
        trade_notional=sum(float(premiums.get(leg.trading_symbol, 0.0)) * leg.total_quantity for leg in intent.legs),
    )
    if not risk.allowed:
