...
C1: tags: list = field(default_factory=list),
...
class Playbook:
    ...
    )  # uuid4 at CREATION (user action, not engine path)
    name: str = ""
    ...
    def __post_init__(self) -> None:
        ...
        # C1: defensive — ensure mutable defaults not shared
        # (field(default_factory) already guarantees)
        ...
        if isinstance(self.structure_type, dict):
            for direction, struct in self.structure_type.items():
                if struct not in VALID_STRUCTURES:
                    logger.warning(
                        "Playbook %r: unknown structure %r for %s", self.name, struct, direction
                    )
        elif isinstance(self.structure_type, str):
            if self.structure_type not in VALID_STRUCTURES:
                logger.warning(
                    "Playbook %r: unknown structure_type %r", self.name, self.structure_type
                )
